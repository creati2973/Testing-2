...
const DAILY_GIFT_MIN = 1;
const DAILY_GIFT_MAX = 15;
...
function register(bot) {
  ...
  bot.action('menu:support', async (ctx) => {
    ...
    const settings = await settingsDb.getSettings();
    await ctx.editMessageText(
      `📩 *SUPPORT*\n\nHaving an issue? Contact us: ${settings?.support_contact || '@NexsusModSupport'}`,
      { parse_mode: 'Markdown', ...kb.mainMenu() }
    );
  });
  ...
  bot.action('menu:addbalance', async (ctx) => {
    await ctx.answerCbQuery();
    await ctx.editMessageText(
      '💵 *ADD BALANCE*\n\nTo add balance, simply purchase any plan via Buy Now — ' +
      'you\'ll be shown the UPI QR to pay there. Balance top-ups without a purchase ' +
      'can be enabled by the admin on request.',
      { parse_mode: 'Markdown', ...kb.mainMenu() }
    );
  });
