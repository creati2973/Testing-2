...
async function requireAdmin(ctx, next) {
  const isAdmin =
    (await users.isAdmin(ctx.from.id)) ||
    String(ctx.from.id) === String(process.env.SUPER_ADMIN_ID);
  if (!isAdmin) {
...
function register(bot) {
  ...
  bot.on('text', async (ctx, next) => {
    ...
    const isAdminUser =
      (await users.isAdmin(ctx.from.id)) || String(ctx.from.id) === String(process.env.SUPER_ADMIN_ID);
