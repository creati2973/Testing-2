...
function register(bot) {
  ...
  // ---- Approve / Reject balance top-up ----
  bot.action(/^admin:topup:approve:(\d+)$/, requireAdmin, async (ctx) => {
    await ctx.answerCbQuery();
    const topupId = Number(ctx.match[1]);
    const topup = await topupsDb.getTopup(topupId);
    if (!topup) return ctx.reply('⚠️ Top-up not found.');
    if (topup.status !== 'verifying') {
      return ctx.reply(`⚠️ Top-up #${topupId} is not awaiting verification (status: ${topup.status}).`);
    }

    await users.creditBalance(topup.user_id, Number(topup.amount), 'topup', `Balance top-up #${topupId} approved`, null);
    await topupsDb.setTopupStatus(topupId, 'approved');

    await ctx.editMessageCaption(`✅ Top-up #${topupId} approved by admin.`, {}).catch(() => {});

    await ctx.telegram.sendMessage(
      topup.user_id,
      `✅ Your balance top-up of ₹${Number(topup.amount).toFixed(2)} has been approved and added to your wallet!`
    );
  });

  bot.action(/^admin:topup:reject:(\d+)$/, requireAdmin, async (ctx) => {
    await ctx.answerCbQuery();
    const topupId = Number(ctx.match[1]);
    const topup = await topupsDb.getTopup(topupId);
    if (!topup) return ctx.reply('⚠️ Top-up not found.');

    await topupsDb.setTopupStatus(topupId, 'rejected');
    await ctx.editMessageCaption(`❌ Top-up #${topupId} rejected.`, {}).catch(() => {});

    await ctx.telegram.sendMessage(
      topup.user_id,
      `❌ Your balance top-up request could not be verified. Please contact support if this is a mistake.`
    );
  });
  ...
  bot.action('admin:pendingtopups', requireAdmin, async (ctx) => {
    await ctx.answerCbQuery();
    const pending = await topupsDb.listTopupsByStatus('verifying', 20);
    if (!pending.length) return ctx.reply('No balance top-ups pending verification. ✅');

    let text = '💵 *Pending Balance Top-Ups:*\n\n';
    pending.forEach((t) => {
      text += `#${t.id} — ₹${t.amount} — user ${t.user_id}\n`;
    });
    await ctx.reply(text, { parse_mode: 'Markdown' });
  });
  ...
  // ---- Support contact/link setup ----
  bot.action('admin:setsupport', requireAdmin, async (ctx) => {
    await ctx.answerCbQuery();
    state.set(ctx.from.id, { action: 'admin_setsupport_text' });
    await ctx.reply(
      '📩 Send the support contact to display (e.g. "@YourSupportUsername"):'
    );
  });

  // ---- Updates channel link setup ----
  bot.action('admin:setupdatelink', requireAdmin, async (ctx) => {
    await ctx.answerCbQuery();
    state.set(ctx.from.id, { action: 'admin_setupdatelink_text' });
    await ctx.reply(
      '📥 Send the link for your updates channel (e.g. "https://t.me/YourChannel"), or "-" to remove it:'
    );
  });

  // ---- How-to-use link setup ----
  bot.action('admin:sethowto', requireAdmin, async (ctx) => {
    await ctx.answerCbQuery();
    state.set(ctx.from.id, { action: 'admin_sethowto_text' });
    await ctx.reply(
      '❗ Send the link for your how-to-use tutorial (e.g. a YouTube link), or "-" to remove it:'
    );
  });
  ...
  bot.on('text', async (ctx, next) => {
    ...
    if (s.action === 'admin_setsupport_text') {
      const contact = ctx.message.text.trim();
      state.set(ctx.from.id, { action: 'admin_setsupport_link', contact });
      return ctx.reply(
        '📩 Now send a tappable support link (e.g. "https://t.me/YourSupportUsername"), or "-" to skip having a link button:'
      );
    }

    if (s.action === 'admin_setsupport_link') {
      const linkInput = ctx.message.text.trim();
      const link = linkInput === '-' ? null : linkInput;
      await settingsDb.updateSettings({ support_contact: s.contact, support_link: link });
      state.clear(ctx.from.id);
      return ctx.reply(`✅ Support contact updated: ${s.contact}${link ? `\n🔗 Link: ${link}` : ''}`);
    }

    if (s.action === 'admin_setupdatelink_text') {
      const linkInput = ctx.message.text.trim();
      const link = linkInput === '-' ? null : linkInput;
      await settingsDb.updateSettings({ update_channel_link: link });
      state.clear(ctx.from.id);
      return ctx.reply(link ? `✅ Updates channel link set: ${link}` : '✅ Updates channel link removed.');
    }

    if (s.action === 'admin_sethowto_text') {
      const linkInput = ctx.message.text.trim();
      const link = linkInput === '-' ? null : linkInput;
      await settingsDb.updateSettings({ howto_link: link });
      state.clear(ctx.from.id);
      return ctx.reply(link ? `✅ How-to-use link set: ${link}` : '✅ How-to-use link removed.');
    }
